...
namespace Palestra
{
    public partial class addEditIscritto : Form
    {
        ...
        public addEditIscritto(String operazione, int indice, SqlConnection c, Utilities u, Iscritti i)
        {
            InitializeComponent();
            if (operazione == "edit")
            {
                ...
                if (Int32.Parse(txtEta.Text) >= 10 && Int32.Parse(txtEta.Text) <= 80)
                {
                    ...
                    if (cal1.SelectionRange.Start.ToString() != "")
                    {
                            ...
                            txtNome.Text.Trim().Length > 20 || txtCognome.Text.Trim().Length > 20)
                        {
                            ...
                            this.Controls.Clear();
                            this.InitializeComponent();
                            addEditIscritto ae = new addEditIscritto("edit",id, conn, utilities, iscritti);
                            this.Close();
                            ae.Show();
                        }
                        else
                        {
                            ...
                            String query = "UPDATE Iscritti SET Nome=@Nome, Cognome=@Cognome, Eta=@Eta, Indirizzo=@Indirizzo, Provincia=@Provincia, " +
                                "CAP=@CAP, Citta=@Citta, Immagine=@Immagine WHERE IdIscritto="+id;
        ...
        private void btnFoto_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.OK && op.FileName != "")
            {
                ...
                imgProfilo.ImageLocation = percorso;
            }
            else
            {
        ...
        private void btnEliminaFoto_Click(object sender, EventArgs e)
        {
            percorso = "";
            imgProfilo.ImageLocation = percorso;
        }
